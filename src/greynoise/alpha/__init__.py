...
def query_ip(ip, key=False):
    ...
    """
    r = requests.post(
        "http://api.greynoise.io:8888/v1/query/ip", ({"ip": ip, "key": key})
    )
    if r.status_code == 200:
...
def query_tag(tag, key=False):
    ...
    """
    r = requests.post(
        "http://api.greynoise.io:8888/v1/query/tag", ({"tag": tag, "key": key})
    )
    if r.status_code == 200:
        if "records" in r.json():
            return r.json()["records"]
...
def query_tags(tags, key=False):
    ...
    :type ips: list
    :return:
        Combined list of IP addresses associated with each in list of GreyNoise tags.
    :rtype: list
    """
    tags_list = []
    for tag in tags:
