        ...
        # multi-searches
            ...
            if results["seen"] or ("count" in results and results["count"] > 0):
                ...
                for field in CONTEXT_FIELDS:
                    print(" %s: %s" % (CONTEXT_FIELDS[field], results[field]))
                print()
                print(" " * 10 + "METADATA:")
                print(" " + "-" * 28)
...
def single_ip(api_key, query, type):
    try:
        ...
        if type == "raw" or not type:
            ...
            r = requests.get(url, headers={"key": api_key})
...
def multi_query(api_key, input_file):
    try:
        if input_file:
...
def bulk_query(api_key, date=False):
